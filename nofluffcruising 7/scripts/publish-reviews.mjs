#!/usr/bin/env node
/**
 * No Fluff Cruising — publish video companion posts.
 *
 * The Cowork automation writes each finished post into its Notion tracker row as a
 * fenced code block. This job reads Notion, finds rows whose video is Posted and whose
 * post has not been published yet, and writes the markdown into src/content/reviews/.
 *
 * Notion is the only credential this needs. The Google Drive copy stays as the human
 * editing surface; Notion is what actually ships.
 *
...
const CHANNELS = ['No Fluff Cruising', 'PHAPD']; // PHAPD is the same channel pre-rebrand
...
/** Rows whose video is live and which therefore should have a post on the site. */
...
/** The Cowork automation leaves the shippable markdown in a code block on the row. */
...
function validate(md) {
  ...
  // The internal review header must never reach the live site.
